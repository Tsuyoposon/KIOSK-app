...
Vagrant.configure(2) do |config|
  ...
  config.vm.hostname = "project-rails"
  config.vm.network "forwarded_port", guest: 3000, host: 3000
  config.vm.network "private_network", ip: "192.168.33.10"

  config.vm.synced_folder ".", "/home/Rails_app/current"
...
FLUSH PRIVILEGES;
CREATE USER 'hashigo'@'localhost';
SET PASSWORD FOR 'hashigo'@'localhost' = PASSWORD('password');
CREATE DATABASE hashigo_development DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;
CREATE DATABASE hashigo_test DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;
GRANT ALL PRIVILEGES ON hashigo_development.* to 'hashigo'@'localhost';
GRANT ALL PRIVILEGES ON hashigo_test.* to 'hashigo'@'localhost';
SQL
